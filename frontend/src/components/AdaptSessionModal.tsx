...
interface AdaptSessionModalProps {
  hasApiKey: boolean
  sessionSnapshot: SessionSnapshot
  onClose: () => void
}

export function AdaptSessionModal({ hasApiKey, sessionSnapshot, onClose }: AdaptSessionModalProps) {
  ...
  async function handleSubmit() {
    ...
    setErrorMsg(null)
    try {
      const data = await api.post<{ suggestions: string }>('/ai/adapt-session', {
      ...
      <div className="bg-white w-full sm:max-w-lg rounded-t-2xl sm:rounded-2xl shadow-xl flex flex-col max-h-[90vh]">
        ...
        <div className="flex-1 overflow-y-auto px-4 py-4 space-y-4">
          {!hasApiKey ? (
            ...
            <>
              <p className="text-sm text-slate-500">
                Describe what's going on — an ache, low energy, equipment that's unavailable — and
                get specific suggestions for today's session.
              </p>

              <textarea
                rows={3}
                placeholder="e.g. My lower back is tight today. What should I modify or skip?"
                value={userMessage}
        ...
        {hasApiKey && (
          <div className="px-4 pb-4 pt-2 border-t border-slate-100 shrink-0 flex gap-3">
            {status !== 'success' ? (
              ...
              <button
                type="button"
                onClick={() => { setStatus('idle'); setSuggestions(null); setUserMessage('') }}
